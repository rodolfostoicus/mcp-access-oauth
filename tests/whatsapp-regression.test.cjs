...
test("business diagnostic observes ADMIN and ownership without promoting denied account access or disclosing unrelated assets", async () => {
  ...
  const result = toolPayload(await h.invoke("meta_get_token_permissions", { ...businessInput, include_account_inventory: true }));
...
for (const edge of ["system_users", "owned_ad_accounts"]) test(`rate limit on ${edge} stops subsequent business edges without retry`, async () => {
